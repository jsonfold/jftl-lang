...
| 2 — Foreach | `foreach` (or the `map`/`filter` shortcuts) | Iterate over an array, object, or numeric range |
...
## Stage 2 shortcuts: `map` and `filter`

`map` and `filter` are shorthand for the two most common `foreach` shapes — transform every item, or keep only some of them — without writing out a full `foreach` block.
...
- **`map`** — `{"$": true, "map": EXPR}` is sugar for `{"$": true, "foreach": { "out": EXPR } }`: evaluate `EXPR` once per item and produce the resulting array.
- **`filter`** — `{"$": true, "filter": COND}` is sugar for `{"$": true, "foreach": { "if": COND, "out": "$_" } }`: keep each item, unchanged, whose `COND` is truthy; drop the rest.

Like `foreach`, both act on `_` at the point they run — Stage 1's result, or whatever `data` replaced it with. There's no separate source key here: unlike `foreach`, which has its own `in`, `map`/`filter` always operate on the current value directly.

**`map` and `filter` require the current value to be an array.** This is stricter than full `foreach`, which also accepts an object (iterating key/value pairs) or an integer (iterating a numeric range) — passing `map`/`filter` an object or an integer is an error, not a silently-adapted iteration the way `foreach` would handle it.

`map` and `filter` are mutually exclusive with `foreach`, `case`, and `out` — combining either of them with any of those three is a compile-time error. They only cover the fixed, single-expression shape shown above: anything needing `case`-style branching, custom `var`/`key` bindings, `update` accumulation, or `start`/`stop`/`limit` windowing still needs the full `foreach` form.

### Examples

```json
{ "$": true, "data": "$.scores", "map": "$py=_ * 2" }
```

Input: `{"scores": [1, 2, 3]}` → Output: `[2, 4, 6]`

```json
{ "$": true, "data": "$.scores", "filter": "$py=_ >= 80" }
```

Input: `{"scores": [95, 82, 61]}` → Output: `[95, 82]`
...
Names a structural transformation applied to the `case`/`out` result. See `transformation.md` for the available transforms (`flatten`, `merge`, `object_to_pairs`, `pairs_to_object`, `kvs_to_object`, `values_to_map`, `drop_missing`, `join`) and what each expects/produces. Only runs if the current value isn't `Missing`.
...
`map: "EXPR"` and `filter: "COND"` (see "Stage 2 shortcuts" above) are alternatives to `foreach`, not additional keys alongside it — a logic element uses `foreach`, or `map`, or `filter`, never more than one.

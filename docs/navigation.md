...
#### `$<` across component calls (`function` / `macro`)

Calling a component (`{"$": "componentName", ...}`, see `defs.md`) also creates a new evaluation frame, so `$<` behaves differently depending on the component's `type` — and it behaves **differently from what you might expect**, because it does not simply mean "the caller":

* **`macro`** — the macro body's frame is a child of the *calling* frame. `$<` inside a macro is the caller's current data at the call site — the same value `$` held in the caller, immediately before the call. This is what most people expect `$<` to mean.

* **`function`** — the function body's frame is a child of the **top-level frame**, not the calling frame, regardless of how deeply nested the call is. `$<` inside a function is therefore the input document's current data — the same value `$^` would give — and it skips over the immediate caller entirely.

Both `function` and `macro` still receive the caller's current data as their own `$` (a function's frame explicitly copies it in), so `$` behaves the same either way. Only `$<` differs.

```json
{
  "defs": {
    "tag_macro":    { "type": "macro",    "out": { "seen_via_lt": "$<" } },
    "tag_function": { "type": "function", "out": { "seen_via_lt": "$<" } }
  },
  ...
  "main": {
    ...
    "data": "$.product",
    "out": {
      "from_macro":    { "$": "tag_macro" },
      "from_function": { "$": "tag_function" }
    }
  }
}
```
...
```json
{
  ...
  "from_function": {
    "seen_via_lt": { "product": { "id": "p1" }, "other": "ignored" }
  }
}
```

`main` narrows its current data to `$.product` before making either call. The macro's `$<` sees that narrowed value (`{"id": "p1"}`), because its frame's parent is `main`'s own frame. The function's `$<` sees the full, un-narrowed input document instead, because its frame's parent is the top-level frame — `data: $.product` never touched it.

> **Component parameters are not reached through `$<`.** Values passed as call parameters (`{"$": "greet", "name": "$.customer"}`) become ordinary variables inside the component, looked up with `$name`, exactly like any `set` variable — never with `$<`. `$<` only ever exposes *current data*, never variables.
...
* `defs.md` — Component (`function`/`macro`) definitions and calls, including how `$<` behaves across a call boundary.
